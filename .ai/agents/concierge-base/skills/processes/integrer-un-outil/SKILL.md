...
3. **Adapter le principe générique** (de `base-et-vos-outils-ia.md`) à cet outil, couche par couche:
...
4. **Garder la couture de vérification.** Rappelle la règle: la génération peut être automatique, la validation reste tenue par un humain. Aide l'utilisateur à calibrer, étape par étape, ce qui s'automatise et ce qui attend une approbation.
