...
1. **Ground first.** Run `understand-state` (or read `specs/` and the matrix) so the plan rests
   on what exists, not on memory.
2. **Ask only design-changing questions.** Three at most, each with concrete options; if visual,
   an ASCII mock-up per option. Record the answers in the plan's *Origin* section. If the user
   proposes something better, take it.
...
4. **Keep slices vertical, green, irreversible.** Each one crosses all layers and ships observable
   behaviour; each ends with every gate green; each kills the code it replaces in the same slice.
