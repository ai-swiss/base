...
> «Pour vous situer: combien de process environ, et avez-vous le sentiment que le routage se trompe parce qu'ils sont trop nombreux? Si c'est un petit ensemble, je vous déconseille d'installer quoi que ce soit: la Voie 1 fera mieux le travail.»
...
Si l'utilisateur ne sait pas, propose de regarder ensemble; ne présume de rien.
...
3. **Vérifie qu'Ollama répond** avant d'aller plus loin. Si tu as un terminal, propose d'exécuter une commande de vérification simple (lister les modèles installés, par exemple, selon la doc courante); sinon, demande à l'utilisateur de confirmer qu'Ollama tourne.
...
2. **Exemples illustratifs, non prescriptifs** (à vérifier et à adapter, ils ont pu changer): un embedding multilingue léger de la famille `qwen3-embedding` (petite taille), et un petit raffineur de la famille `qwen3`. Propose-les comme un point de départ raisonnable, non comme une obligation; l'utilisateur reste libre de choisir les siens (un autre embedding à contexte long, par exemple, ou un raffineur d'une autre famille).
3. **Télécharge les deux** avec la commande de téléchargement d'Ollama (consulte la doc pour la forme exacte de la commande et le nom de balise courant de chaque modèle). Si tu as un terminal, propose de l'exécuter, en montrant chaque commande d'abord; sinon, donne les commandes à l'utilisateur.

> «Pour le rôle d'embedding comme pour le rôle de raffineur, voici deux modèles locaux légers qui conviennent bien aujourd'hui. Je vérifie qu'ils sont toujours recommandés, je vous montre les commandes de téléchargement, et vous restez libre d'en choisir d'autres.»
...
3. **La règle tout-ou-rien:** la Voie 2 ne s'active que lorsque **les deux** modèles sont renseignés. Un seul ne change rien, et BASE reste sur la Voie 1.

Variante sans Studio: les mêmes valeurs figurent dans le bloc `routing` du fichier `.ai/studio.settings.json` (`embedding_model`, `refiner_model`, et `k` optionnel). Le serveur valide la règle tout-ou-rien à l'écriture.
...
Propose une vraie demande de l'utilisateur et montre où elle route, pour confirmer que la Voie 2 répond. En cas d'échec (un modèle injoignable, par exemple), rappelle que BASE **retombe automatiquement sur la Voie 1**: jamais de blocage, jamais de silence.
