...
newcomer can find their way without reading everything. It avoids line numbers (they rot); module and
type names are stable enough to grep. Revisit it a couple of times a year, not on every change.
...
and inspectable; when BASE itself calls a remote model, an egress check withholds the resources you marked
confidential (or any resource of a `local-only` root) and says so in the trace. The default policy is
permissive, and BASE polices only its own remote surfaces, not other tools on your machine. There is **no
