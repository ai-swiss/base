...
`diagnose(root)` is a **pure projection** over data that already exists — the resource inventory, the link graph, eval runs (`.ai/experiments/runs`), per-resource mtimes, and open frictions (`.ai/feedback/`) — handed to `diagnoseData(...)`, which returns findings `[{severity, type, path, message, fix_hint}]` and touches no disk. Thirteen finding kinds — `dead_link` (error, broken Markdown links only — an illustrative `code` path is not a link), `orphan` (error under `.ai/agents/`, where unreferenced knowledge is invisible; `warn` elsewhere, where reachability is the documentation graph's concern), `review_due` (warn), `expired` (error), `stale_eval` (warn), `open_friction` (warn), `missing_tool_artifacts` (warn), `recurring_abstention` (warn), `stale_routing_vectors` (warn), `stale_generated_projection` (warn — an adopted, banner-carrying generated file differs from today's `base build` output; the hint names the exact rebuild command), plus the three lenses migrated from `entretien`: `weak_routing`, `missing_description`, `stale_marker` (each warn) — each carrying a mandatory fix hint and one of two severities. The CLI `base doctor [--json]` and Studio's `GET /api/doctor` both call `diagnose(root)`; the CLI renders the findings (`formatDiagnosis`) and exits `1` on any error-severity finding. Source: `tools/doctor/diagnose.mjs`.

**`entretien` vs `doctor`:** the doctor is the one owner of folder health; `entretien` is **deprecated** (its structural lenses — weak routing, missing descriptions, dormant markers — now live in `diagnoseData` too). The 1.x surface is honoured unchanged (same report, same `--json` shape, same exit code) with a referral line on the text door; the verb is removed in the next minor version. The raw marker QUERY stays with `base markers`/`list_markers`.
