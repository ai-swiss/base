...
`change_id` = `chg_<12 hex>` derived from `hash(target + "\n" + base_hash + "\n" + content)`. Including `base_hash` means two proposals for the same target+content but against **different base states** get distinct ids, so a pending change is not silently overwritten by a later one. The commit TOCTOU guard (step 4 below) still re-checks `base_hash` independently.
