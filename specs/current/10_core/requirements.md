...
| ID | Requirement |
|---|---|
| FR-CLI-001 | Commands: `validate`, `index`, `discover`, **`route`**, **`route-test`**, `inventory`, `open`, **`context`**, `access`, `invoke`, `init`, `doctor`, `entretien`, `trace`, `help`, **`propose`**, **`commit`**, **`promote`**, **`markers`**, **`build`**, `docs`, `studio`, `whereis`, `update`. Matches `tools/base.mjs`. See `routing.md`, `writes.md`, `build.md`, `maintenance.md`, and FR-INIT-* / FR-DOCTOR-* / FR-DOCS-*. |
...
| ID | Requirement |
|---|---|
| FR-MCP-001 | Tools: `load_agent`, `discover_resources`, **`route_request`**, `open_resource`, **`get_context_pack`**, `access_resource`, `invoke_tool`, **`propose_change`**, **`commit_change`**, **`promote_resource`**, **`list_markers`**, **`report_friction`** (write-gated, FR-FEEDBACK-003). Router primitives delegate to the broker via `base-core-adapter`. Matches `mcp/src/index.ts`. |
