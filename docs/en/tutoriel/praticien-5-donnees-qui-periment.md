...
2. Open `infos/agenda.md`: the `valid_until` field carries a past date (that's on purpose,
   for the exercise).
...
💡 **Why it worked**: expertise ages. `valid_until` declares the lifespan of a piece of reference data; `base doctor` checks these dates against your files to spot what is going stale, without running anything, by simple reading. Maintenance becomes visible.
