...
Until now, you read Veytaux's tourist office only once it was complete. Now you build YOUR project: a real one, independent, outside the repository.

1. From the repository (where `base` = `node .ai/base.mjs`, see [step 0](harnais.md)), create your
   project elsewhere. `init` first shows what it would create, without writing anything yet:
...
2. Enter your project. From now on, here, `base` = `node .ai/base.mjs`. Everything keeps working even
   once you have left the repository, because `init` noted where the engine lives:

   ```
   cd ~/mon-office-tourisme
   ```

Rather than copy the finished corpus, you fill in a skeleton: the effort makes you understand.

3. `init` created an agent named after your folder (`mon-office-tourisme`). Give it a first
   process in `.ai/agents/mon-office-tourisme/skills/processes/renseigner-un-visiteur/SKILL.md`. Start
...
🆘 **Common failures**: *`base init` says "Déjà un BASE"*: the folder already contains a `.ai/agents/`, so choose an empty folder. *`base route` fails from your project*: are you really IN `~/mon-office-tourisme` (where `base` = `node .ai/base.mjs`)? *validate fails on the frontmatter*: no tabs, no `|` or `{}` in YAML; keep the form of the skeleton.
