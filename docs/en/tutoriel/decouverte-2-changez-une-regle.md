...
**You will**: confirm that the assistant follows your files, not some hidden memory, proven by the ✅ below.
...
💡 **Why it worked**: the assistant has no database and no hidden memory; it READS your Markdown files on every task. Change the file and its behavior changes with it. The new conversation forces it to reread; in the old one, it could keep the old price in mind.
