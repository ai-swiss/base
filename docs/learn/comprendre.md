...
status: active
sensitivity: public
keywords: [comprendre, approche, co-pensee, contexte, logiciel-numerique, comportement, verification]
---

# Comprendre BASE et façonner l'interaction avec l'IA

Travailler sérieusement avec l'IA, c'est accepter qu'elle produise vite, mais qu'elle se trompe parfois avec aplomb: l'enjeu est de garder la maîtrise de ce que vous signez, sans renoncer à la vitesse. Cette page montre comment BASE structure cette collaboration pour que la qualité tienne dans la durée, que vous soyez indépendant, PME ou service public. Vous y verrez **pourquoi** cette structure est nécessaire, **comment** un agent fonctionne, et **comment en créer un** pour votre métier.
...
BASE ne part pas d'une préférence pour un outil, mais d'un constat: l'IA générative produit sans peine, mais la qualité durable dépend de ce qui entoure cette production. Ce qui prime, c'est la souveraineté sur votre savoir et l'articulation de l'ensemble: contexte, mémoire, processus, permissions et décisions humaines. Dans cette structure, la vérification est un savoir-faire, jamais une garantie.

L'approche est donc institutionnelle avant d'être technique. Elle cherche à rendre explicite ce qui, dans bien des usages de l'IA, reste implicite: qui sait quoi, qui décide quoi, quelles données sont utilisées, quelles actions sont permises, et comment reprendre le travail plus tard.

La difficulté tient à ce que cette technologie ne ressemble pas seulement aux logiciels numériques classiques. Un logiciel traditionnel expose des écrans, des menus, des boutons, des formulaires et des règles codées à l'avance. Un modèle de langage produit plutôt un comportement: il répond, reformule, infère, imite des raisonnements, suit parfois une méthode, oublie parfois une contrainte, et donne souvent une impression de continuité humaine. Cette impression ne doit pas être confondue avec une conscience, une intention ou une compréhension garantie. Elle suffit pourtant à changer la manière de travailler.

Pour travailler avec ce comportement, l'image la plus utile est celle d'un **collègue venu d'ailleurs, amnésique: il a une représentation riche du monde, mais pas du vôtre**. Côté représentation: il connaît des domaines vérifiables, plus sûr là où l'entraînement est dense comme le code ou les mathématiques, et il sait lire, écrire, généraliser et proposer. Côté contexte: il ne connaît ni votre terrain, ni vos clients, ni vos règles implicites. Cette image n'est pas une définition, mais un repère pour décider plus justement: comment articuler un design, où placer la vérification, comment structurer le savoir qu'on lui confie. Deux traits, propres au modèle, la complètent. D'abord, sa mémoire n'est pas partagée par défaut: chaque conversation repart de zéro. Ensuite, le langage qui le pilote reste sous-spécifié: une même consigne se comprend de plusieurs façons. Ces deux traits sont à la fois une force (souplesse, capacité à généraliser) et une faiblesse (oubli, ambiguïté). Il faut donc lui donner une mémoire de travail, des processus, des critères de vérification et des limites d'action. Ce récit, et les pertes de contrôle qu'il permet d'éviter, est développé dans [Co-penser avec l'IA, pourquoi BASE](co-penser-avec-lia.md).

### Le problème

La plupart des gens utilisent l'IA comme un interlocuteur sans structure: on ouvre un agent conversationnel, on pose une question, on obtient une réponse. Cela fonctionne pour des questions ponctuelles, mais montre vite ses limites:
...
- **Rien n'est capitalisé.** Ni historique, ni structure, ni réutilisation.
- **Vous ne savez pas quand elle se trompe.** L'IA produit des réponses fluides et assurées, même quand elles sont fausses. Sans contexte structuré, vous n'avez aucun repère pour juger la qualité du résultat.
...
**Le principe fondamental**: une réponse de l'IA est une proposition à examiner avant d'en faire une conclusion. Souvent elle est juste; parfois elle est fausse avec aplomb. Votre rôle est de cadrer, d'évaluer et d'ajuster, en boucle, jusqu'à obtenir quelque chose de fiable. Les fichiers sont la source de vérité.
...
C'est pourquoi BASE repose sur des **fichiers**, et non sur des conversations. Une conversation disparaît quand vous fermez l'outil. Un fichier reste. Un journal de session prolonge cette mémoire d'une conversation à l'autre.

*Ce qui se passe quand on l'ignore:* chaque session recommence à zéro. L'utilisateur se répète. L'agent repose des questions déjà résolues. Le travail ne s'accumule pas.
...
C'est pourquoi les marqueurs `[A VALIDER]`, `[DECISION]` sont structurés et cherchables. «Qu'est-ce qui est en attente?» trouve sa réponse en une seconde, même après des mois.
...
Vous dites à l'agent: «Ne modifie jamais les fichiers du cadre.» Au bout de 30 minutes de conversation, l'agent oublie cette consigne et modifie un fichier qu'il ne devrait pas toucher.

C'est pourquoi les garde-fous critiques sont **mécaniques** (permissions, protections), au-delà du seul texte. Une permission qui bloque mécaniquement ne dérive jamais, quelle que soit la longueur de la conversation.

*Ce qui se passe quand on l'ignore:* les protections textuelles fonctionnent dans les conversations courtes, mais s'effritent dans les conversations longues. L'agent dépasse ses limites par simple oubli progressif du contexte, sans aucune malveillance.

### 5. Certaines actions ne se défont pas

Un devis envoyé ne se «dé-envoie» pas. Un fichier client créé avec de mauvaises données peut propager l'erreur. Un engagement pris sur un prix est un engagement.

C'est pourquoi les **points de décision** existent avant chaque action irréversible. Le point de décision sépare «on réfléchit» de «on agit». C'est une **friction productive**: un coût délibéré qui prévient des erreurs bien plus coûteuses.
...
C'est pourquoi BASE cherche à rendre la vérification *légère* sans la supprimer: la structure allège l'effort de contrôle, elle ne remplace pas votre compréhension. Vous pouvez déléguer le détail, jamais la capacité de juger. Recharger régulièrement la vue d'ensemble (relire en profondeur, discuter en équipe de ce qui a été produit et pourquoi) fait partie du travail.

*Ce qui se passe quand on l'ignore:* la vérification devient un simple tampon. Les erreurs passent: on vérifie encore, mais on a perdu l'intuition qui fait sentir quand une vérification s'impose.

### 8. Ce qu'on ne peut ni emporter ni auditer finit par vous échapper

Votre savoir vit dans un dispositif à plusieurs couches: vos fichiers, et les instructions invisibles de l'outil (prompt système, règles, politiques du fournisseur). Si tout votre contexte est captif d'une interface, et si vous ignorez ce qui façonne réellement le comportement de l'IA, vous travaillez sans souveraineté.

C'est pourquoi BASE met votre savoir dans des fichiers lisibles, portables et auditables: vous restez souverain sur votre couche, et vous gardez de quoi partir. La portabilité est une condition du contrôle, pas un simple confort.
...
Elles valent pour toute coordination entre des entités distinctes qui ne partagent ni les mêmes ressources, ni les mêmes modes de fonctionnement, ni la même façon de se tromper: deux humains collaborant à distance, un humain et une IA, ou toute autre configuration. Les outils changent, les modèles changent; ces contraintes structurelles, elles, demeurent.

---

## La dette de vérification

Produire avec l'IA demande désormais peu d'effort; s'assurer qu'une réponse est juste relève d'un autre travail, qui dépend de la tâche: sur les terrains à vérificateur externe (code, mathématiques, schéma), l'erreur se détecte seule et l'IA va loin en autonomie; ailleurs, le vérificateur, c'est vous, et une structure forte rend cette vérification légère plutôt que lourde (faute de quoi la dette de vérification s'accumule). Cette asymétrie entre produire et vérifier est le constat qui fonde BASE, et c'est elle qui rend la structure indispensable.
...
La dette s'accumule en silence. Elle se révèle au pire moment: quand un client conteste un montant, quand un candidat relève une incohérence, quand un partenaire pointe une erreur.

**Structure forte en amont → vérification légère en aval.** C'est pourquoi BASE structure avant de générer: des fichiers métier à jour, des connaissances précises, des marqueurs explicites. Plus la structure est forte, plus la vérification est légère. Plus la structure est faible, plus la dette de vérification enfle.
...
> **Pourquoi «agents» et «skills»?** Ce sont les noms les plus répandus aujourd'hui, et les modèles d'IA les reconnaissent nativement: BASE les réutilise par **pragmatisme**. Ce qui compte, c'est ce qui se cache derrière le vocabulaire:
...
Tous les skills sont des fichiers texte au format SKILL.md. Ce format est lisible par tous les modèles et reconnu nativement par certains outils IA; dans les autres, l'agent peut ouvrir les fichiers explicitement. Chaque skill comporte des métadonnées en en-tête (frontmatter YAML) et un contenu en Markdown.

BASE distingue deux types de skills:

**Les processes** (workflows invocables): des conversations structurées que l'utilisateur déclenche. «Créer un devis» → l'agent suit le process étape par étape, avec des reformulations (pour vérifier la compréhension) et des points de décision (avant chaque action irréversible).

**Les compétences** (connaissances réutilisables): des fiches d'expertise que l'agent consulte quand le workflow ou la demande le justifie. La terminologie des devis, les règles de TVA, les conventions de communication. Les compétences servent dans plusieurs processes: c'est leur raison d'être.
...
**Reformulation** (légère, friction faible): l'agent résume ce qu'il a compris. L'utilisateur corrige ou confirme. Se tromper est sans conséquence: on ajuste et on continue. Fréquent.

**Point de décision** (critique, friction productive): l'agent est prêt à créer un fichier ou à modifier des données. L'utilisateur confirme explicitement. Agir sans confirmation risquerait de créer des données incorrectes difficiles à corriger. Rare et important.

La distinction est essentielle. Si chaque étape devient un point de décision, l'attention se dilue et le mécanisme perd son pouvoir protecteur. Les reformulations sont légères et fréquentes. Les points de décision sont rares, explicites, et réservés aux moments qui comptent.

### Les marqueurs

Du texte structuré, inséré dans les documents générés, qui rend l'état du travail cherchable. Leur forme fixe en fait des repères que l'œil saisit d'emblée et qu'un script peut traiter automatiquement: les compter, les lister, les regrouper.
...
Les marqueurs correspondent aux étapes de la boucle de co-pensée: `[A COMPLETER]` apparaît pendant le cadrage, `[A VALIDER]` quand l'agent confie une proposition, `[ATTENTION]` pendant l'évaluation, `[DECISION]` après l'ajustement. Après des mois d'utilisation, ils permettent de retrouver instantanément tout ce qui est en attente, tout ce qui a été décidé, et pourquoi.

### Le journal

Mémoire entre sessions. L'agent écrit une entrée à la fin de chaque workflow dans `.ai/journal/`. Quand vous revenez le lendemain, il lit le journal et sait où il en est. Sans journal, chaque session recommence de zéro, et la nécessité 1 est violée.
...
Les fichiers texte sont un choix structurel délibéré, non un réflexe technique:

- **Lisibles par les humains ET les machines.** Nul besoin d'un outil spécial pour lire un fichier Markdown, ni d'une API pour accéder à vos données. Ouvrez le fichier, tout est là.
...
- **Durables.** Les bases de données changent de format. Les API disparaissent. Les plateformes ferment. Un fichier texte écrit en 2026 sera lisible en 2046.
...
Les outils IA évoluent vite. Les modèles changent. Les interfaces se renouvellent. Mais vos skills, vos templates et vos données métier demeurent. **Votre structure de connaissances est votre vrai capital.**

Le format SKILL.md est avant tout un contrat textuel lisible. Si un outil le prend en charge nativement, l'expérience est plus fluide. Sinon, un SKILL.md reste un fichier Markdown que l'agent peut lire explicitement.

### Configuration outil

Pour que votre outil IA charge l'agent et découvre ses skills avec le moins de friction possible, il faut une configuration propre à cet outil. Certains automatisent une partie du chargement, d'autres demandent de pointer manuellement vers `AGENT.md`. Chaque outil a besoin de 5 choses:
...
| **Protection du cadre** | Réduire ou bloquer la modification de `.ai/` selon l'outil | Les instructions du cadre ne doivent pas être modifiées par accident |

Le créateur d'assistant consulte la documentation à jour de l'outil pour proposer la bonne configuration. Si l'outil n'est pas connu, l'agent guide l'utilisateur vers une configuration manuelle.
...
**Niveau 2: mécanique.** Permissions, protections, règles dans la configuration de l'outil, ou des actions passant par un connecteur BASE. Quand un garde-fou est critique et que les conséquences d'un oubli sont lourdes, le niveau mécanique devient indispensable. Le niveau 2 ne remplace pas le niveau 1: il le renforce là où l'outil le permet.
...
Ce qui rend un assistant IA utile tient moins à la technologie qu'à la **structure des connaissances** que vous lui donnez. Un bon AGENT.md, doté de bons skills, transforme n'importe quel outil IA en assistant spécialisé. Votre expertise est le multiplicateur: l'IA l'amplifie, mais ne la remplace pas.
...
2. **Attention aux trois pièges.** La facilité (demander est facile, obtenir un bon résultat ne l'est pas), l'apparence (un texte bien écrit n'est pas forcément correct), et les promesses exagérées des vendeurs. Voir [La co-pensée en pratique](pratiques-co-pensee.md).
...
4. **Commencer petit.** Un agent doté d'un seul workflow qui fonctionne bien vaut mieux que cinq non testés. On peut toujours en ajouter.
5. **Versionner les ressources.** `_v1`, `_v2`, etc. Cela permet de les faire évoluer sans casser ce qui fonctionne.
6. **Copier, pas modifier.** Les templates restent intacts dans `.ai/`. L'agent copie, puis adapte.

### Interagir

7. **Discuter avant d'agir.** L'agent propose, vous validez. Jamais l'inverse.
8. **Une question à la fois.** Les bons workflows avancent pas à pas, non en bloc.
9. **Résumer régulièrement.** Sur les conversations longues, demandez un point d'avancement pour garder le fil.
...
- **LLM = orchestration.** Décider *quoi faire ensuite* revient au modèle dans l'outil, guidé par le texte et les candidats du routeur; rien n'est codé en dur dans l'outillage.

**Règle de conception:** un point d'extension doit protéger une frontière réelle. Mettre du vocabulaire métier dans l'index, ou de l'orchestration métier dans le MCP, est une erreur de conception. C'est pourquoi le routage **vit avec le texte** (`use_when`, descriptions) plutôt que dans un catalogue tenu à la main: un tel catalogue violerait le plan «Texte = vérité».
