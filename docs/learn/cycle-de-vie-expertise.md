...
(livré avec BASE, rattaché au créateur d'assistant) parcourt vos documents (modes d'emploi, wikis,
checklists) et **propose** leur conversion en process, compétences, documents et templates.
Chaque écriture passe par le gate propose → commit: à vous de valider chaque diff.
...
place ailleurs. Chaque suggestion du modèle arrive en diff, que vous appliquez ou refusez. La dette
naît le plus souvent de quelques paragraphes que personne n'a relus: ici, tout reste visible.
...
d'exécuter du code se **déclare** (`report_limitation`) plutôt que de se simuler. L'évaluation prend
toute sa valeur au passage à l'échelle: tout BASE conserve un socle de process écrits et tenus par
les gens, mais quelques-uns sont **promus et institutionnalisés**, et ce sont eux qu'il faut
maintenir sous évaluation.

## 4. Exécuter, chez l'hôte

L'assistant tourne dans un outil IA capable de lire vos fichiers (par exemple GitHub Copilot, Antigravity, Claude Code, Cowork, OpenCode ou Kilo Code), ou tout hôte MCP, avec le broker BASE
comme médiateur: confinement au root, gate d'écriture, trace locale. L'exécution de code demeure
...
se consigne en une phrase: outil MCP `report_friction`, ou un simple «ça n'a pas marché» qui
route vers le process [`signaler-une-friction`](../../.ai/agents/concierge-base/skills/processes/signaler-une-friction/SKILL.md).
Chaque **abstention du routeur** (demande qu'aucun agent ne couvre) se journalise d'elle-même.
Studio présente les deux en pile de travail: une friction est un amendement de process en
attente; une demande restée sans réponse qui revient est un process à créer.

## 6. Vieillir sous surveillance

Un corpus métier se dégrade en silence. Deux champs de cycle de vie (`status`, `review_by`),
deux dates de validité (`valid_from`, `valid_until`): le routeur écarte les ressources
dépréciées, le contexte annonce «périmé depuis le …», et [`base doctor`](../reference/framework-public.md)
relève ce qui menace de rompre: liens morts, ressources orphelines, évaluations périmées, relectures
échues, frictions ouvertes, chacun avec sa piste de correction.

## 7. Gouverner chaque sortie vers un modèle

Avant qu'un octet parte vers un modèle, une seule règle s'applique: une ressource
`confidential`, ou tout un root `local-only`, ne part **jamais** vers un fournisseur distant, et le
refus s'affiche, à l'écran et dans la trace. Voir [Protection des données](../trust/protection-des-donnees.md)
et [les preuves](../trust/evidence.md).

---

La boucle entière tient dans des fichiers qui vous appartiennent. L'importer revient à copier un
dossier, l'auditer se fait avec `base doctor`, et la quitter, c'est partir avec vos fichiers.
Le savoir-faire y demeure indépendant du modèle qui l'exécute: les outils passent, le contexte
reste, et changer de modèle ne réécrit pas l'expertise.
