...
**2. Le routage choisit, il ne récupère pas.** Le plancher déterministe (`base route`, les fixtures, l'usage sans modèle) est lexical et **ne fait aucun appel réseau**; il retourne un agent et un process, ou s'abstient, et sa stabilité est testée par `base route-test`. Dans un outil d'IA, le modèle décide à partir d'une carte de «Quand l'utiliser». Dans les deux cas, on choisit une unité de travail entière, on ne récupère pas de fragments par similarité. Le ranking sémantique reste optionnel et peut tourner localement (par exemple via Ollama); le **modèle** relève du choix de l'utilisateur, hors périmètre de BASE.
